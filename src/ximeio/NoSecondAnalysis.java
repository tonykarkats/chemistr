...
public class NoSecondAnalysis extends JFrame {

    private String[] columnNames = {"Επωνυμο","Όνομα","Παρώνυμο","Διεύθυνση","Σταθερό","Κινητό","Νο Βαρελιου",
                                   "Κιλά","Ημερομηνία 1ης Ανάλυσης"};
    ...
    public NoSecondAnalysis (){
       ...
       //UIManager.put("Text.font", new FontUIResource("Dialog", Font.BOLD, 24));
        ...
        setTitle("ΠΕΛΑΤΕΣ ΜΕ ΕΚΚΡΕΜΟΤΗΤΑ 2ης ΑΝΑΛΥΣΗΣ");
        ...
        panel.add(new JLabel("Σύνολο "+table.getRowCount()));
    }
    ...
    private void initializeTable() {
        ...
        Vector<String> rowData = new Vector<String>();
            try {
                ...
                ResultSet rs = stmt.executeQuery("select Surname,Name,Nickname,Address,Phone1,Phone2,Wine.NrBarrel,Wine.Kilos,DATE_FORMAT(Wine.Added,'%d / %m / %Y') as AnalysisDate "
                        + "from Customer,Wine where extract(year from Wine.Added) = extract(year from CURDATE()) and WineID in (select WineID from winesNotSecondAnalysis)"
                        + " ORDER BY Surname;");
                while (rs.next()) {                    
